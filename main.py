...
# Activation words
ACTIVATION_WORDS = ["robot", "robit", "robort", "rabot", "rawbot", "rowbot", "assistant", "ai", "ei"]
    ...
    while True:
        try:
            ...
            if command_matches(command):
                ...
                objects_text = []
                if left: objects_text.append(f"On the left: {', '.join(left)}")
                if center: objects_text.append(f"In the center: {', '.join(center)}")
                if right: objects_text.append(f"On the right: {', '.join(right)}")

                final_detection_text = " | ".join(objects_text) if objects_text else "No objects detected."
                print(f"📦 Object positions: {final_detection_text}")
                ...
                full_response = f"{description}. {final_detection_text}. " + " ".join(guidance)
